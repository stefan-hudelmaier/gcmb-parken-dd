...
class Adapter:
    ...
    def run_once(self):
        ...
        for city_key, city_info in cities.items():
            ...
            for lot in lots:
                ...
                if free is not None:
                    ...
                    self.mqtt_publisher.send_msg(str(free), topic, retain=True)
                ...
                if total is not None:
                    ...
                    self.mqtt_publisher.send_msg(str(total), topic, retain=True)
                ...
                if state is not None:
                    ...
                    self.mqtt_publisher.send_msg(str(state), topic, retain=True)
                    self.logger.debug(f"Published {state} to {topic}")
                # Publish original JSON per lot
                topic = f"{self.base_topic}/{city_key}/{lot_id}"
                self.mqtt_publisher.send_msg(json.dumps(lot, ensure_ascii=False), topic, retain=True)
            ...
            self.mqtt_publisher.send_msg(str(city_free), topic_free, retain=True)
            self.mqtt_publisher.send_msg(str(city_total), topic_total, retain=True)
        ...
        self.mqtt_publisher.send_msg(str(global_free), topic_global_free, retain=True)
        self.mqtt_publisher.send_msg(str(global_total), topic_global_total, retain=True)
        self.mqtt_publisher.send_msg(str(global_lots), topic_global_lots, retain=True)
